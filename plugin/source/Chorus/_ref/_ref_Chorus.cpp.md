...
Chorus::Chorus(int maxVoicesIn) : maxVoices(juce::jlimit(MIN_MAX_VOICES, MAX_MAX_VOICES, maxVoicesIn)) {
	...
	mix = configure.defaultMix;
	baseDelay = configure.defaultBaseDelayMs;
	sampleRate = 44100.0;
	prepared = false;
	// Apply default voice count and delay type from configuration
	numActiveVoices = std::clamp(configure.defaultVoiceCount, MIN_MAX_VOICES, maxVoices);
	currentDelayType.store(configure.defaultDelayType);
	...
	voices = std::make_unique<Voice[]>(maxVoices);
		...
		for (int i = 0; i < maxVoices; ++i) {
			...
			voices[i].mix = calculateVoiceMixRatio(i); // Decreasing mix for each voice (configured)
			voices[i].baseDelay = configure.voiceBaseDelayMs[std::min(i, MAX_MAX_VOICES - 1)];
			...
			float voiceRate = configure.defaultRateHz;      // base rate
			float voiceDepth = configure.defaultDepth;
		...
		voices[i].delayLines[0] = DelayLine::create(configure.defaultDelayType);
		voices[i].delayLines[1] = DelayLine::create(configure.defaultDelayType);
	...
	currentStereoMode = configure.defaultStereoMode;
	stereoSpread = configure.defaultStereoSpread;
	
	// Initialize mid-side configuration with default values
	midEnabled = configure.defaultMidEnabled;
	sideEnabled = configure.defaultSideEnabled;
	sideGainDb = configure.defaultSideGainDb;
	
	// Initialize filter configuration with default values
	lpfEnabled = configure.defaultLPFEnabled;
	hpfEnabled = configure.defaultHPFEnabled;
	lpfCutoff = configure.defaultLPFCutoffHz;  // 20kHz
	hpfCutoff = configure.defaultHPFCutoffHz;     // 20Hz
	
	updateStereoConfiguration();
}

float Chorus::calculateVoiceMixRatio(int voiceIndex) const {
	// Read from configured voice mix array with bounds protection
	const int idx = std::clamp(voiceIndex, 0, MAX_MAX_VOICES - 1);
	return configure.voiceMix[idx];
}
...
void Chorus::updateParameters(const UpdateArgs& args) {
	// Apply only provided fields
	if (args.rate) setRate(*args.rate);
	if (args.depth) setDepth(*args.depth);
	if (args.mix) setMix(*args.mix);
	if (args.baseDelayMs) setBaseDelay(*args.baseDelayMs);
	if (args.voiceCount) setVoiceCount(*args.voiceCount);
	if (args.stereoMode) setStereoMode(*args.stereoMode);
	if (args.stereoSpread) setStereoSpread(*args.stereoSpread);
	if (args.midEnabled) setMidEnabled(*args.midEnabled);
	if (args.sideEnabled) setSideEnabled(*args.sideEnabled);
	if (args.sideGainDb) setSideGain(*args.sideGainDb);
	if (args.lpfEnabled) setLPFEnabled(*args.lpfEnabled);
	if (args.lpfCutoffHz) setLPFCutoff(*args.lpfCutoffHz);
	if (args.hpfEnabled) setHPFEnabled(*args.hpfEnabled);
	if (args.hpfCutoffHz) setHPFCutoff(*args.hpfCutoffHz);
	if (args.delayType) setDelayType(*args.delayType);
}
